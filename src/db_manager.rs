...
pub mod db_manager {
    ...
    pub fn read_stock(conn: &mut PooledConn, location: Option<Procedencia>) -> Vec<Existencia> {
        ...
        match location {
            Some(pr) => match pr {
                Procedencia::Casa => {
                    let list: Vec<(i32, i32)> = conn
                        .query_map(
                            "SELECT id_objeto, cantidad FROM existencias_home",
                ...
                Procedencia::Tara => {
                    let list: Vec<(i32, i32)> = conn
                        .query_map(
                            "SELECT id_objeto, cantidad FROM existencias_tara",
                            |(obj, quantity)| (obj, quantity),
                        )
                        .unwrap();
                    for o in list {
            ...
            None => {
                let list_home: Vec<(i32, i32)> = conn
                    .query_map(
                        "SELECT id_objeto, cantidad FROM existencias_home",
                        |(obj, quantity)| (obj, quantity),
                    )
                ...
                let list_tara: Vec<(i32, i32)> = conn
                    .query_map(
                        "SELECT id_objeto, cantidad FROM existencias_home",
                        |(obj, quantity)| (obj, quantity),
                    )
